...
public class SubFiloPesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public SubFiloBean Subfilo = new SubFiloBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				SubFiloPesquisaView dialog = new SubFiloPesquisaView(
	...
	public SubFiloPesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa SubFilo");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (SubFiloBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(), filo.getFilo().getNome() });
						}
					} catch (Exception e) {
				...
				case 1: {
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							SubFiloBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<SubFiloBean> lista = ifc
									.pesquisar(nome, false);
							dtm.setRowCount(0);
							for (SubFiloBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
										filo.getNome(),
										filo.getFilo().getNome() });
							}
						} catch (Exception e) {
							JOptionPane.showMessageDialog(rootPane,
									"Erro na pesquisa !");
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						SubFiloBean filos = ifc.pesquisar(ids);
						Subfilo.setId(filos.getId());
						Subfilo.setNome(filos.getNome());
						Subfilo.setFilo(filos.getFilo());
