...
public class VariedadePesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public VariedadeBean Variedade = new VariedadeBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				VariedadePesquisaView dialog = new VariedadePesquisaView(
	...
	public VariedadePesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa Variedade");
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				case 0: {
					try {
						...
						List<VariedadeBean> lista = ifc.pesquisar();
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (VariedadeBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(),
									filo.getSubEspecie().getNome() });
						}
					} catch (Exception e) {
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							InterfaceVariedadeController ifc = ControllerFactory
									.getInstance()
									.getInterfaceVariedadeController();
							VariedadeBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							...
							dtm.addRow(new Object[] { elemento.getId(),
									elemento.getNome(),
									elemento.getSubEspecie().getNome() });
						} catch (Exception e) {
							JOptionPane.showMessageDialog(rootPane,
									"Erro na pesquisa !");
						}
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<VariedadeBean> lista = ifc.pesquisar(nome,
									true);
							dtm.setRowCount(0);
							for (VariedadeBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						VariedadeBean filos = ifc.pesquisar(ids);
						Variedade.setId(filos.getId());
						Variedade.setNome(filos.getNome());
						Variedade.setSubEspecie(filos.getSubEspecie());
