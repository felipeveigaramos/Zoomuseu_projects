...
public class SubEspeciePesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public SubEspecieBean SubEspecie = new SubEspecieBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				SubEspeciePesquisaView dialog = new SubEspeciePesquisaView(
	...
	public SubEspeciePesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa SubEspecie");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (SubEspecieBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(), filo.getEspecie().getNome() });
						}
					} catch (Exception e) {
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							InterfaceSubEspecieController ifc = ControllerFactory
									.getInstance()
									.getInterfaceSubEspecieController();
							SubEspecieBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<SubEspecieBean> lista = ifc.pesquisar(nome,
									true);
							dtm.setRowCount(0);
							for (SubEspecieBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						SubEspecieBean filos = ifc.pesquisar(ids);
						SubEspecie.setId(filos.getId());
						SubEspecie.setNome(filos.getNome());
						SubEspecie.setEspecie(filos.getEspecie());
