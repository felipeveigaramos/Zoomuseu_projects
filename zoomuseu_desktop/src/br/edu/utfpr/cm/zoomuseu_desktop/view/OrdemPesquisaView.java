...
public class OrdemPesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public OrdemBean Ordem = new OrdemBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				OrdemPesquisaView dialog = new OrdemPesquisaView(
	...
	public OrdemPesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa Ordem");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (OrdemBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(),
									filo.getInfraClasse().getNome() });
						}
					} catch (Exception e) {
				...
				case 1: {
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							OrdemBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							...
							dtm.addRow(new Object[] { elemento.getId(),
									elemento.getNome(),
									elemento.getInfraClasse().getNome() });
						} catch (Exception e) {
							JOptionPane.showMessageDialog(rootPane,
									"Erro na pesquisa !");
						}
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<OrdemBean> lista = ifc.pesquisar(nome, true);
							dtm.setRowCount(0);
							for (OrdemBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
										filo.getNome() });
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						OrdemBean filos = ifc.pesquisar(ids);
						Ordem.setId(filos.getId());
						Ordem.setNome(filos.getNome());
						Ordem.setInfraClasse(filos.getInfraClasse());
