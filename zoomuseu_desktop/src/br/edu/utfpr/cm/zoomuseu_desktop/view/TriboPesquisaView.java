...
public class TriboPesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public TriboBean Tribo = new TriboBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				TriboPesquisaView dialog = new TriboPesquisaView(
	...
	public TriboPesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa Tribo");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (TriboBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(),
									filo.getSubFamilia().getNome() });
						}
					} catch (Exception e) {
				...
				case 1: {
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							TriboBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							...
							dtm.addRow(new Object[] { elemento.getId(),
									elemento.getNome(),
									elemento.getSubFamilia().getNome() });
						} catch (Exception e) {
							JOptionPane.showMessageDialog(rootPane,
									"Erro na pesquisa !");
						}
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<TriboBean> lista = ifc.pesquisar(nome, true);
							dtm.setRowCount(0);
							for (TriboBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
										filo.getNome() });
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						TriboBean filos = ifc.pesquisar(ids);
						Tribo.setId(filos.getId());
						Tribo.setNome(filos.getNome());
						Tribo.setSubFamilia(filos.getSubFamilia());
