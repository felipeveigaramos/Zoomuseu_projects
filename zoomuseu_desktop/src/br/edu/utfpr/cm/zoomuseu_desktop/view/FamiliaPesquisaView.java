...
public class FamiliaPesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public FamiliaBean Familia = new FamiliaBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				FamiliaPesquisaView dialog = new FamiliaPesquisaView(
	...
	public FamiliaPesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa Familia");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (FamiliaBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(),
									filo.getSuperFamilia().getNome() });
						}
					} catch (Exception e) {
				...
				case 1: {
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							FamiliaBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							...
							dtm.addRow(new Object[] { elemento.getId(),
									elemento.getNome(),
									elemento.getSuperFamilia().getNome() });
						} catch (Exception e) {
							JOptionPane.showMessageDialog(rootPane,
									"Erro na pesquisa !");
						}
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<FamiliaBean> lista = ifc.pesquisar(nome, true);
							dtm.setRowCount(0);
							for (FamiliaBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
										filo.getNome() });
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						FamiliaBean filos = ifc.pesquisar(ids);
						Familia.setId(filos.getId());
						Familia.setNome(filos.getNome());
						Familia.setSuperFamilia(filos.getSuperFamilia());
