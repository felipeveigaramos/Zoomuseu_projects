...
public class GeneroPesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public GeneroBean Genero = new GeneroBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				GeneroPesquisaView dialog = new GeneroPesquisaView(
	...
	public GeneroPesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa Genero");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (GeneroBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(), filo.getTribo().getNome() });
						}
					} catch (Exception e) {
				...
				case 1: {
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							GeneroBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<GeneroBean> lista = ifc.pesquisar(nome, true);
							dtm.setRowCount(0);
							for (GeneroBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
										filo.getNome() });
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						GeneroBean filos = ifc.pesquisar(ids);
						Genero.setId(filos.getId());
						Genero.setNome(filos.getNome());
						Genero.setTribo(filos.getTribo());
