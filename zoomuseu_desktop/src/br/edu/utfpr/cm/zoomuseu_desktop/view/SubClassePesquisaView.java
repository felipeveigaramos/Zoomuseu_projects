...
public class SubClassePesquisaView extends JDialog {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	public SubClasseBean SubClasse = new SubClasseBean();
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				SubClassePesquisaView dialog = new SubClassePesquisaView(
	...
	public SubClassePesquisaView(java.awt.Frame parent, boolean modal) {
		super(parent, modal);

		setTitle("Pesquisa SubClasse");
				...
				case 0: {
					try {
						...
						DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
								.getModel();
						dtm.setRowCount(0);
						for (SubClasseBean filo : lista) {
							dtm.addRow(new Object[] { filo.getId(),
									filo.getNome(), filo.getClasse().getNome() });
						}
					} catch (Exception e) {
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							InterfaceSubClasseController ifc = ControllerFactory
									.getInstance()
									.getInterfaceSubClasseController();
							SubClasseBean elemento = ifc.pesquisar(id);
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
					...
					if (jTPesquisa.getText().equals("")) {
						...
						try {
							...
							DefaultTableModel dtm = (DefaultTableModel) jTBPesquisa
									.getModel();
							dtm.setRowCount(0);
							List<SubClasseBean> lista = ifc.pesquisar(nome,
									true);
							dtm.setRowCount(0);
							for (SubClasseBean filo : lista) {
								dtm.addRow(new Object[] { filo.getId(),
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER
					...
					if (-1 != index) {
						...
						SubClasseBean filos = ifc.pesquisar(ids);
						SubClasse.setId(filos.getId());
						SubClasse.setNome(filos.getNome());
						SubClasse.setClasse(filos.getClasse());
